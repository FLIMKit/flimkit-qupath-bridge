...
`Phasor` opens the phasor window on the image in view. Cursors placed on the plot report pixel counts and phase and modulation lifetimes, and come back as classified QuPath annotations carrying those measurements. `GET /v1/phasor/settings` describes what the window can set, in the same `{values, schema}` shape `GET /v1/fit/defaults` uses, so the settings dialog is generated rather than laid out by hand.
