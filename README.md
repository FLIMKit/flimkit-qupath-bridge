...
The FLIM image itself gains a channel per fitted map. QuPath will not let a live server change its channel count or pixel type, `isCompatibleMetadata` refuses both, so the extension reopens the image after a per-pixel fit. It comes back declaring intensity in photons plus a channel per `tau` map in nanoseconds, as float32, and the tiles come from `planes/stack.tif` with the same crop and downsample a single plane would use. Before any fit there is one uint16 intensity channel, exactly as before.
